...
PATH_TO_SPEECHBOX = "src/speechbox"
...
def read_init():
    ...
    with open(os.path.join(PATH_TO_SPEECHBOX, "__init__.py"), "r", encoding="utf-8", newline="\n") as f:
        lines = f.readlines()
...
def check_dummies(overwrite=False):
    ...
    path = os.path.join(PATH_TO_SPEECHBOX, "utils")
    dummy_file_paths = {
    ...
    for backend in dummy_files.keys():
        if dummy_files[backend] != actual_dummies[backend]:
            if overwrite:
                print(
                    f"Updating speechbox.utils.dummy_{short_names.get(backend, backend)}_objects.py as the main "
                ...
                raise ValueError(
                    ...
                    f"speechbox.utils.dummy_{short_names.get(backend, backend)}_objects.py. Run `make fix-copies` "
                    "to fix this."
                )
