...
class Restorer:
    def __init__(
        self, model: WhisperForConditionalGeneration, processor: WhisperProcessor
    ):
    ...
    def from_pretrained(cls, pretrained_model_or_path):
        model = WhisperForConditionalGeneration.from_pretrained(
            pretrained_model_or_path, low_cpu_mem_usage=True
        )
    ...
    def get_punctuation_tokens(self):
        ...
        for i in tqdm.tqdm(range(len(self.processor.tokenizer))):
            if self.processor.tokenizer.convert_ids_to_tokens(i) in list(
                string.punctuation
            ):
    ...
    def __call__(
        ...
        decoder_start_ids = torch.tensor(decoder_start_tokens, dtype=torch.long)[
            None, :
        ]
        current_ids = decoder_start_ids.broadcast_to(
            encoder_hidden_states.shape[:1] + decoder_start_ids.shape[1:]
        )

        for l_w, u_w, up_w in zip(
            target_lower_words, target_upper_words, target_all_upper_words
        ):
            is_finished = False
            while not is_finished:
                logits = self.model(
                    decoder_input_ids=current_ids, encoder_outputs=encoder_hidden_states
                ).logits[:, -1]
